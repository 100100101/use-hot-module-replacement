...
const checkIsDoubleSaveDiscard_1 = tslib_1.__importDefault(require("./checkIsDoubleSaveDiscard"));
...
//     throw err
// }
exports.default = ({ watching, collectDependencies, ignore, setHMRHooks, parents, doubleSaveDiscardMs, }) => {
    return async (path) => {
        if (ignore(path))
            return;
        if (watching[path])
        ...
        const watchCallback = async (eventType, filename) => {
            ...
            if (doubleSaveDiscardMs > -1) {
                const isDoubleSaveDiscard = await (0, checkIsDoubleSaveDiscard_1.default)(path, doubleSaveDiscardMs);
                if (isDoubleSaveDiscard)
                    return;
            }
            ...
            for (let d = 0; d < deps.length; ++d) {
                for (let l = 0; l < deps[d].length; ++l) {
                    ...
                    reloaded[path] = true;
                    const oldModule = require.cache[path];
                    const oldModuleHot = oldModule.hot;
                    ...
                    try {
                        ...
                        for (const parentPath in parentAcceptedDependencies) {
                            const parentAcceptedModule = require.cache[parentPath];
                            const parentAcceptedModuleHot = parentAcceptedModule.hot;
                            const childDisposeHandlers = parentAcceptedModuleHot._childDisposeDependencies;
                            for (const childDisposeHandler of Object.values(childDisposeHandlers)) {
                            ...
                            parentAcceptedModuleHot._childDisposeDependencies =
                                {};
                            const acceptedDependency = parentAcceptedModuleHot._acceptedDependencies[path];
        ...
        watching[path] = (0, node_watch_1.default)(path, { persistent: false }, watchCallback);
    };
};
//# sourceMappingURL=useStartWatching.js.map